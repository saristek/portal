...
<div class="h-full flex flex-col justify-between overflow-y-auto">
	<div id="head_main" class="h-10 flex font-semibold justify-between items-stretch bg-gray-300">
		<button class="bg-blue-300 px-2" on:click|preventDefault={() => goto('/dashboard/sync/employee')}>« kembali</button>
		<span class="p-2">Jenis Ke-Pegawaian</span>
	</div>
								...
								<tbody>
									{#each coba as item, id}
										<tr>
											...
											<th scope="col" class="px-2 py-3 text-center font-light text-xs">
												<button
													id={item.id}
